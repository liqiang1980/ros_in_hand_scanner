        ...
        run (bool use_device, bool use_file)
        {
          if (use_file)
          {
            ...
            if (use_device)
            {
              ...
              boost::function<void (const pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr&)> f = boost::bind (&NormalEstimation_WITHGPU::file_cloud_cb<Device>, this, _1);
              filegrabber->registerCallback (f);
            }
            else
            {
//               std::cerr << "[NormalEstimation] Using CPU..." << std::endl;
//               boost::function<void (const pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr&)> f = boost::bind (&NormalEstimation_WITHGPU::file_cloud_cb<Host>, this, _1);
//               filegrabber->registerCallback (f);
            }
...
//             {
//               std::cerr << "[NormalEstimation] Using GPU..." << std::endl;
//               boost::function<void (const boost::shared_ptr<openni_wrapper::Image>& image, const boost::shared_ptr<openni_wrapper::DepthImage>& depth_image, float)> f = boost::bind (&NormalEstimation_WITHGPU::cloud_cb<Device>, this, _1, _2, _3);
